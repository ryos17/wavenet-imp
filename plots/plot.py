...
def _plot_waveform_overlap_on_ax(
    ax,
...
def plot_waveform_overlaps() -> None:
    panels = [
        (
            "Not pruned",
            ROOT
            / "models/sparsity_level_sweep/output-b40-lr0.001-e1500-2026-03-09_18-18-33-026461",
        ),
        (
            "IMP",
            ROOT
            / "models/sparsity_level_sweep/output-b40-lr0.001-e1500-p90-local-exponential-ps10-pe750-2026-03-10_10-41-36-427138",
        ),
        (
            "One-shot",
            ROOT / "models/one_shot_sweep/p90",
        ),
    ]

    fig, axes = plt.subplots(3, 1, figsize=(7.0, 6.0), sharex=True)
    for ax, (_title, run_dir) in zip(axes, panels):
        _plot_waveform_overlap_on_ax(
            ax,
    ...
    fig.tight_layout()
    fig.savefig(OUT_DIR / "waveform_overlap_stacked_ieee.png")
    plt.close(fig)
